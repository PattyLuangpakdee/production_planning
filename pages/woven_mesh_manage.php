...
include "header.php";
if (isset($_GET['mode']) == "") {
    $SQL_NEWID = "SELECT Max(substr(woven_mesh_id,-7))+1 as MaxID FROM tb_woven_mesh";
    $QUERY_NEWID = mysqli_query($conn, $SQL_NEWID);
    $ARRAY_NEWID = mysqli_fetch_array($QUERY_NEWID);
    if ($ARRAY_NEWID['MaxID'] == '') {
        $NEWID = "WM-0000001";
    } else {
        $NEWID = "WM-" . sprintf("%07d", $ARRAY_NEWID['MaxID']);
    }
} elseif (isset($_GET['mode']) == "edit") {
    $SQL_EDIT = "SELECT * FROM tb_woven_mesh WHERE woven_mesh_id = '" . $_GET['woven_mesh_id'] . "'";
    $QUERY_EDIT = mysqli_query($conn, $SQL_EDIT);
    $ARRAY_EDIT = mysqli_fetch_array($QUERY_EDIT); 
    $NEWID = $_GET['woven_mesh_id'];
}
?>
<div class="content">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card ">
                            ...
                            <div class="card-body ">
                                <form method="POST" action="woven_mesh_save.php?mode=<?=$_GET['mode']?>" class="form-horizontal" autocomplete="OFF">
                                    <fieldset>
                                        <div class="form-group">
                                            <div class="row">
                                                <label class="col-sm-2 control-label">รหัส มตฐ.ขนาดตา</label>
                                                <div class="col-sm-6">
                                                    <input type="text" name="woven_mesh_id" value="<?=$NEWID?>" readonly="" class="form-control">
                                                </div>
                                            </div>
                                        </div>
                                    </fieldset>
                                    ...
                                    <fieldset>
                                        <div class="form-group">
                                            <div class="row">
                                                <label class="col-sm-2 control-label">วัด(ตา)</label>
                                                <div class="col-sm-6">
                                                    <input type="number" step=any name="Measure_eye"  value="<?=$ARRAY_EDIT['Measure_eye']?>" class="form-control" required="">
                                                </div>
                                            </div>
                                        </div>
                                    </fieldset>
